...
if (!class_exists('RapidAddon')) {
	...
	//		echo "</pre>";
				...
				if (!empty($this->fields)){
					foreach ($this->fields as $field_slug => $field_params) {
						...
						switch ($field_params['type']) {
							...
							case 'images':
								$dynamicsep = ',';
								$values = $parsedData[$field_slug][$index];
								if ( ! empty( $values ) ) {
									...
									$urls = explode( $dynamicsep, $values );
									$car_images = array();
									if(!empty($urls)) {
										foreach ( $urls as $key => $url) {

											$image_url_or_path = $url;
											...
											$car_images[] = $uploaded_image;
										}
										//set _thumbnail_id for sell car funcitonality
										update_post_meta( $post_id, '_thumbnail_id' ,$car_images[0] );
									}
									update_field( 'car_images', $car_images, $post_id );
							...
							case 'stemp_image':
								// import the specified image, then set the value of the field to the image ID in the media library
								$image_url_or_path = $parsedData[$field_slug][$index];
								if ( ! empty( $image_url_or_path ) ) {
									$download = 'yes';
									global $wpdb;
									$sql = $wpdb->prepare(
										"SELECT post_id FROM $wpdb->postmeta
											 WHERE meta_key = '_wp_attached_file' AND meta_value LIKE '%s'",
										'%/'.basename($image_url_or_path)
									);
									$exits_attachment_id = $wpdb->get_var( $sql );
									if ( isset($exits_attachment_id) && ! empty($exits_attachment_id) ) {
										$uploaded_image = $exits_attachment_id;
									} else {
										$uploaded_image = PMXI_API::upload_image($post_id, $image_url_or_path, $download, $importData['logger'], true, "", "images", true, $importData['articleData']);
									...
									$review_stamp_limit_opt = Redux::get_option( 'car_dealer_options', 'review_stamp_limit' );
									$review_stamp_limit     = isset( $review_stamp_limit_opt ) ? $review_stamp_limit_opt : 1;

									for ( $i = 1; $i <= $review_stamp_limit; $i++ ) {
										if ( "review_stamp_logo_" . $i == $field_slug ) {
											update_field( 'review_stamp_logo_' . $i, $uploaded_image, $post_id );
										}
									}
								}
								break;
							case 'brochure_upload':
								// import the brochure PDF File
								$pdf_file_url_or_path = $parsedData[$field_slug][$index];
								if ( ! empty( $pdf_file_url_or_path ) ) {

									$download       = 'yes';
									$uploaded_file = PMXI_API::upload_image($post_id, $pdf_file_url_or_path, $download, $importData['logger'], true, "", "files", true, $importData['articleData']);
									...
									update_field( 'pdf_file', $uploaded_file, $post_id );
								}
								break;
							case 'file':
								$image_url_or_path = $parsedData[$field_slug][$index];
		...
		function render_field($field_params, $field_slug, $current_values, $in_the_bottom = false){
			...
			} else if ($field_params['type'] == 'images') {
			...
			} else if ( $field_params['type'] == 'stemp_image' ) {
				PMXI_API::add_field(
					'simple',
					$field_params['name'],
					array(
			...
			} else if ( $field_params['type'] == 'brochure_upload' ) {
				PMXI_API::add_field(
					'simple',
					$field_params['name'],
					array(
